...
export default {
  ...
  beforeMount() {
    ...
    // 在发请求之前，把接口需要传递的参数，进行整理，把参数整理好服务器就会返回查询的数据
  ...
  computed: {
    // mapGetters里面的写法传递的是数据，因为getter计算是没有划分模块【home、search】
  ...
  methods: {
    ...
    // 删除分类的名字
    removeCategoryName() {
      // 带给服务器的参数置空，还需要给服务器再发一次请求
      // 带给服务器的参数说明(可有可无的) 如果属性值为空的字符串还是会把相应的参数带给服务器
      // 但把相应字段变成underfind，当前这个字段不会带给服务器
    ...
    // 排序的操作(初始状态，默认综合降序)
    changeOter(fla) {
      // fla 形参：他是一个标记，代表用户点击的是(综合还是价格),用户点击传递过来的
      let originOrder = this.searchParams.order
      // 获取的是最开始的状态
      let originFla = originOrder.split(':')[0]
      let originSort = originOrder.split(':')[1]
      // 准备一个新的order属性值
      let newOrder = ''
      if (fla == originFla) {
        // 点击的一定是综合
        newOrder = `${originFla}:${originSort == 'desc' ? 'asc' : 'desc'}`
      } else {
        // 点击的是价格
        newOrder = `${fla}:${'desc'}`
      }
      // 将新的outer赋值给searchParams
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起ajax请求
      this.getData()
      // 每一次请求完毕，应该把1、2、3级分类的id清除，接收下一次的数据
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
    },
